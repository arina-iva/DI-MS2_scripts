...
comparison_table_180EG <- read.csv("./180_settings_comparison/180E+G/180EG_comparison_table_inprog.csv")
...
iter <- 1 #<- TO CHANGE!!! 
comparison_table_180EG$settings[iter] <- str_match(file_names[iter], "180E\\+G_(.*?)\\.mzML")[,2]
str_match(file_names[iter], "180E\\+G_(.*?)\\.mzML")[,2]
...
ms2Scan <- c(1:81) #41 for 0.04 Da step, 17 for 0.1 Da step nd 137 for 2 Da wide window
...
template_chimera <- averageSpectra(filtered_spectra, (TIC_max-1):(TIC_max+1), dev*2)


# 2. Load and process blank
blankExp <- openMSfile("./180_settings_comparison/blank180_stepped_NCE_3.mzML") # <= for stepped NCE
blankExp <- openMSfile("./180_settings_comparison/blank180_separate_NCE_3.mzML") # <= for separate NCEs
...
##For NCE 50, use the highest intensity fragment as a proxy for the precursor 180E, fragment: 102.0549 (column 5)
corr_matrix_prec <- corr_matrix[ ,c(5, ncol(corr_matrix))]%>%
  filter(if_any(everything(), ~ . > 0))

## for NCE 65,  use the highest intensity fragment as a proxy for the precursors. 180E fragment:102.0549 (column 7), 180G fragment: 135.0441 (col 11)
corr_matrix_prec <-  as.data.frame(corr_matrix[ ,c(7, 11)])%>%
  filter(if_any(everything(), ~ . > 0))
...
model_1 <- lm(comp_180E$mz_err_Da ~ comp_180E$mz_measur)
summary(model_1)
comp_180E <- comp_180E %>%
...
comp_180G <-ref_180G_defNCE_int %>%
  ...
  mutate(mz_err_Da = mz_measur-mz_theor)%>%
  group_by(mz_theor)%>%
  slice_max(order_by = int_reconstr, n = 1)%>%
  ungroup()

plot(x = comp_180G$mz_measur, y = comp_180G$mz_err_Da)

##change the mz value fix model type: linear, log, or quadratic
model_2 <- lm(comp_180G$mz_err_Da ~ (comp_180G$mz_measur + I(comp_180G$mz_measur^2)))
...
comparison_table_180EG$Isobar_2_unassign_peaks[iter] <- length(comp_180G$int_ref) - sum(!is.na(comp_180G$int_reconstr)&!is.na(comp_180E$mz_theor))
...
write.csv(mirror_plot_data_180G, file = paste0("./180_settings_comparison/180E+G_", 
...
library(patchwork)
m_p_180EG_El <- m_p_E+m_p_G
m_p_180EG_El 
save(m_p_180EG_El, file =  paste0("./180_settings_comparison/180E+G_", 
                                         str_match(file_names[iter], "180E\\+G_(.*?)\\.mzML")[,2], "_180EG_mirrorplot.RData" ))
