...
iter <- 1 #<- TO CHANGE!!! 
comparison_table_342AB$settings[iter] <- str_match(file_names[iter], "342A\\+B_([^_]+)_")[,2]
str_match(file_names[iter], "342A\\+B_([^_]+)_")[,2]
...
ref_342A <- read.csv("./342_settings_comparison/ref_342A.csv")
...
## For Low mass resolution (R = 15000 / 30000 / 60000) use a a fragment as a proxy fpor 342A 123.044 (col 10/ col 9 / col 9)
corr_matrix_prec <- corr_matrix[ ,c(9, ncol(corr_matrix))]%>%
  as.data.frame()%>%
  filter(if_any(everything(), ~ . > 0))

## For NCE 50, use the highest intensity fragment as a proxy for the precursor 342A, fragment: 123.044 (col 11)
corr_matrix_prec <- corr_matrix[ ,c(11, ncol(corr_matrix))]%>%
 as.data.frame()%>%
  filter(if_any(everything(), ~ . > 0))

## for NCE 65,  use the highest intensity fragment as a proxy for the precursors.  fragment: 342B fragment: 108.0570 (column 15), 342A fragment: 123.044 (col 20); 
corr_matrix_prec <- corr_matrix[ ,c(20,15)]%>%
  as.data.frame()%>%
  filter(if_any(everything(), ~ . > 0))
...
model_2 <- lm(comp_342B$mz_err_Da ~ comp_342B$mz_measur)
summary(model_2)
comp_342B <- comp_342B %>%
